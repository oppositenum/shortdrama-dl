...
界面上的地址存进 `barkUrl`；留空就完全不推，一次请求都不发。`notify.js` 只做纯粹的地址整理和发送，什么时候推的策略留在 `main.js`：

- 地址容错：只填 key 补 `https://api.day.app/`，缺协议补 `https://`，去掉尾斜杠。照官方示例整条粘进来（`前缀/标题/内容`）时，官方域名只保留第一段 key——标题和内容由程序自己拼；自建服务器的子路径原样保留。非 http(s) 协议一律拒掉。
- 标题和内容按路径段转义，内容里的 `/` 和 `?` 不会截断推送。
- 推送失败只在日志里留一行，绝不影响下载；日志里的地址会把 key 打码。
- 完成通知只在真的凑齐全集（`fullyComplete`）时发，批量跑几百部时它就是「这部可以看了」的信号。
- 出错通知走 `log(..., 'error')`，第一条立刻推，其后 5 分钟内的攒起来合并，任务收尾时补发；用户主动取消不算故障。`pushBark()` 内部只能用 `send('download:log')`，用 `log()` 会被 error 分支绕回来变成死循环。
